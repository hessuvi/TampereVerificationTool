...
%%% $Source: Src/Illustrator/WebSrc/gui.w $
%%% $Id: gui.w 1.1 Mon, 25 Aug 2003 15:49:29 +0300 hvi $

\part{User interface}

\chapter{User guide}

Currently program is started from command line using command
\begin{center}
  \verb|illux <lts file> [<style file>]|
\end{center}
where \verb<lts file< is name of file in format described in
\cite{ALA:ltsformat}.  Optional second argument \verb<style file< is
filename which content describes visible actions and they appearance
in picture. File contains one line for each
actions in form
\begin{center}
  \verb|<action name> <color> [<dash pattern>]|
\end{center}
Action name is given in same format as in LTS file. For example
\begin{center}
  \verb<|send ack|<
\end{center}
Color is given in format acceptable by Tcl/Tk. For example, color can
be symbolic color name (red, black, green \(\dots\)) or in RGB-format in
form 
\begin{center}
  \verb|#RRRRGGGGBBBB|
\end{center}
where \verb|RRRR|, \verb|GGGG|, and \verb|BBBB|
are intensities of red, green and blue color components given as one,
two, three or four digit hex numbers.

Optional \verb|<dash pattern>| is bit pattern to dash line. It is in
form 
\begin{center}
  \verb|" xxx x xxx "|
\end{center}
where bits marked with \verb|x|
represent given foreground color and spaces background color.

If no second argument is given and project wide style file (see below)
is not found, all transitions is draw using solid dash pattern and
randomly selected colors.

\verb|illux| is capable to draw labeled transition systems containing
approximately 20 -- 30 states, depending how transitions connect them.


\subsection*{Project wide style file}

Names of actions are unique in project or design level. So it is
convenient to use single style file automatically for all visualized
labelled tranisitions systems.

In unix system all related files are usually kept in single%
\marginpar{\small\textsf{This is implemented, if there is
easy way to get needed information from operating system.
pwd,dirname,basename ?}}
subdirectory.  Visualization tool assumes that name of project is same
as name of subdirectory, where visualized LTS file is found. Style
file name is project name appended by prefix \verb<.acs< (comes from
\verb<actions<).


\chapter{Plain algorithm}
  ...
  for( IlLTS::StateIterator ss = lts.beginStates() ;
	...
	if( ss->flags() & IlStateBase::INITIAL_STATE )
	{
	    piirros.TulostaTila( ss->coords(), stateName,
				 INITIAL_STATE_COLORS, cut,
				 statePropNames );
	}
	else if( !(ss->flags() & IlStateBase::HASTRANSITIONS_FLAG) )
	{
	    piirros.TulostaTila( ss->coords(), stateName,
				 DEADLOCK_STATE_COLORS, cut,
				 statePropNames );
	}
      else
	{
	  piirros.TulostaTila( ss->coords(), stateName,
			       NORMAL_STATE_COLORS, cut,
                               statePropNames );
	}
    }
