...
#define CALLOC(ptr, nr)\
  if(!( ptr = (bool)calloc((size_t)(nr), sizeof(*ptr))) && nr) {\
    fprintf(stderr, "calloc of %d bytes failed on line %d of file %s\n",\
            nr * sizeof(*ptr), __LINE__, __FILE__);\
    exit(EXIT_FAILURE);\
  }
...
#define nCALLOC(ptr, nr, toType)\
  if(!( ptr = (toType)calloc((size_t)(nr), sizeof(*ptr))) && nr) {\
    fprintf(stderr, "calloc of %d bytes failed on line %d of file %s\n",\
            nr * sizeof(*ptr), __LINE__, __FILE__);\
    exit(EXIT_FAILURE);\
  }
...
#define MALLOC(ptr, nr)\
  if(!(((void*)ptr) = (malloc((size_t)((nr) * sizeof(*ptr))))) && nr) {\
...
#define MALLOC(ptr, nr)\
  if(!( ptr = (bool)(malloc((size_t)((nr) * sizeof(*ptr))))) && nr) {\
    fprintf(stderr, "malloc of %d bytes failed on line %d of file %s\n",\
            nr * sizeof(*ptr), __LINE__, __FILE__);\
    exit(EXIT_FAILURE);\
  }

#define nMALLOC(ptr, nr, toType)\
  if(!( ptr = (toType)(malloc((size_t)((nr) * sizeof(*ptr))))) && nr) {\
    fprintf(stderr, "malloc of %d bytes failed on line %d of file %s\n",\
            nr * sizeof(*ptr), __LINE__, __FILE__);\
    exit(EXIT_FAILURE);\
  }
...
#define REALLOC(ptr, nr)\
  if(!(((void*)ptr) = realloc(ptr, (size_t)((nr) * sizeof(*ptr)))) && nr) {\
    fprintf(stderr, "realloc of %d bytes failed on line %d of file %s\n",\
            nr * sizeof(*ptr), __LINE__, __FILE__);\
    exit(EXIT_FAILURE);\
    }*/

#define nREALLOC(ptr, nr, toType )\
  if(!( ptr = (toType)realloc(ptr, (size_t)((nr) * sizeof(*ptr)))) && nr) {\
    fprintf(stderr, "realloc of %d bytes failed on line %d of file %s\n",\
            nr * sizeof(*ptr), __LINE__, __FILE__);\
    exit(EXIT_FAILURE);\
  }
#define REALLOC(ptr, nr)\
  if(!( ptr = (bool)realloc(ptr, (size_t)((nr) * sizeof(*ptr)))) && nr) {\
    fprintf(stderr, "realloc of %d bytes failed on line %d of file %s\n",\
            nr * sizeof(*ptr), __LINE__, __FILE__);\
    exit(EXIT_FAILURE);\
  }

#define nMALLOCCPY(nptr, optr, nr, toType )\
  {nMALLOC(nptr, nr, toType ); \
