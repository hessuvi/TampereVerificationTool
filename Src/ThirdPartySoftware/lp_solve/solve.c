...
static short isvalid(lprec *lp)
{
  ...
  if(!lp->row_end_valid) {
    nMALLOC(num, lp->rows + 1, int* );
    nMALLOC(rownum, lp->rows + 1, int* );
  ...
  nCALLOC(rownum, lp->rows + 1, int* );
  nCALLOC(colnum, lp->columns + 1, int* );
...
static void resize_eta(lprec *lp, int min_size)
{
  while(lp->eta_alloc <= min_size)
    lp->eta_alloc += lp->eta_alloc / 2;
  /* fprintf(stderr, "resizing eta to size %d\n", lp->eta_alloc); */
  nREALLOC(lp->eta_value, lp->eta_alloc + 1, double* );
  nREALLOC(lp->eta_row_nr, lp->eta_alloc + 1, int* );
} /* resize_eta */
...
void invert(lprec *lp)
{
  ...
  nCALLOC(rownum, lp->rows + 1, int* );
  nCALLOC(col, lp->rows + 1, int* );
  nCALLOC(row, lp->rows + 1, int* );
  nCALLOC(pcol, lp->rows + 1, double* );
  nCALLOC(frow, lp->rows + 1, short int* );
  nCALLOC(fcol, lp->columns + 1, short int* );
  nCALLOC(colnum, lp->columns + 1, int* );
...
static int solvelp(lprec *lp)
{
  ...
  nCALLOC(drow, lp->sum + 1, double* );
  nCALLOC(prow, lp->sum + 1, double* );
  nCALLOC(Pcol, lp->rows + 1, double* );
  nCALLOC(test, lp->sum +1, short int* ); 
		     ...
		     int     recursive)
{
  ...
  if(failure == OPTIMAL) { /* there is a good solution */
    ...
    if(notint) { /* there is at least one value not yet int */
      ...
      nMALLOC(new_upbo,  lp->sum + 1, double* );
      nMALLOC(new_lowbo, lp->sum + 1, double* );
      nMALLOC(new_lower, lp->sum + 1, short int* );
      nMALLOC(new_basis, lp->sum + 1, short int* );
      nMALLOC(new_bas,   lp->rows + 1, int* );
...
int lag_solve(lprec *lp, REAL start_bound, int num_iter, short verbose)
{
  ...
  /* allocate mem */  
  nMALLOC(OrigObj, lp->columns + 1, double* );
  nCALLOC(ModObj, lp->columns + 1, double* );
  nCALLOC(SubGrad, lp->nr_lagrange, double* );
  nCALLOC(BestFeasSol, lp->sum + 1, double* );
  nMALLOCCPY(old_bas, lp->bas, lp->rows + 1, int* );
  nMALLOCCPY(old_lower, lp->lower, lp->sum + 1, short int* );
