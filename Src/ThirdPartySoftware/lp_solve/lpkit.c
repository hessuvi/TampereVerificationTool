...
lprec *make_lp(int rows, int columns)
{
  ...
  // NEW1: CALLOC(newlp->mat, newlp->mat_alloc);
  nCALLOC(newlp->mat, newlp->mat_alloc, matrec* );
  nCALLOC(newlp->col_no, newlp->mat_alloc + 1, int*);
  nCALLOC(newlp->col_end, columns + 1, int* );
  nCALLOC(newlp->row_end, rows + 1, int* );
  newlp->row_end_valid = FALSE;
  nCALLOC(newlp->orig_rh, rows + 1, double* );
  nCALLOC(newlp->rh, rows + 1, double* );
  nCALLOC(newlp->rhs, rows + 1, double* );

  nCALLOC(newlp->must_be_int, sum + 1, short int* );
  for(i = 0; i <= sum; i++)
    newlp->must_be_int[i] = FALSE;

  nCALLOC(newlp->orig_upbo, sum + 1, double* );
  for(i = 0; i <= sum; i++)
    newlp->orig_upbo[i] = newlp->infinite;

  nCALLOC(newlp->upbo, sum + 1, double* );
  nCALLOC(newlp->orig_lowbo, sum + 1, double* );
  nCALLOC(newlp->lowbo, sum + 1, double* );

  newlp->basis_valid = TRUE;
  nCALLOC(newlp->bas, rows+1, int* );
  nCALLOC(newlp->basis, sum + 1, short int* );
  nCALLOC(newlp->lower, sum + 1, short int* );
  ...
  nCALLOC(newlp->eta_value, newlp->eta_alloc, double* );
  nCALLOC(newlp->eta_row_nr, newlp->eta_alloc, int* );

  /* +1 reported by Christian Rank */
  nCALLOC(newlp->eta_col_end, newlp->rows_alloc + newlp->max_num_inv + 1,
          int* );
  ...
  nCALLOC(newlp->solution, sum + 1, double* );
  nCALLOC(newlp->best_solution, sum + 1, double* );
  nCALLOC(newlp->duals, rows + 1, double* );
  ...
  nCALLOC(newlp->ch_sign, rows + 1, short int* );
...
lprec *copy_lp(lprec *lp)
{
  ...
  nMALLOCCPY(newlp, lp, 1, lprec* ); /* copy all non pointers */

  if(newlp->names_used) {
    nMALLOCCPY(newlp->col_name, lp->col_name, colsplus, char (*)[25] );
    nMALLOCCPY(newlp->row_name, lp->row_name, rowsplus, char (*)[25] );
  }

  newlp->rowname_hashtab = copy_hash_table(lp->rowname_hashtab);
  newlp->colname_hashtab = copy_hash_table(lp->colname_hashtab);

  nMALLOCCPY(newlp->mat, lp->mat, newlp->mat_alloc, matrec* );
  nMALLOCCPY(newlp->col_end, lp->col_end, colsplus, int* );
  nMALLOCCPY(newlp->col_no, lp->col_no, newlp->mat_alloc + 1, int* );
  nMALLOCCPY(newlp->row_end, lp->row_end, rowsplus, int* );
  nMALLOCCPY(newlp->orig_rh, lp->orig_rh, rowsplus, double* );
  nMALLOCCPY(newlp->rh, lp->rh, rowsplus, double* );
  nMALLOCCPY(newlp->rhs, lp->rhs, rowsplus, double* );
  nMALLOCCPY(newlp->must_be_int, lp->must_be_int, sumplus, short int* );
  nMALLOCCPY(newlp->orig_upbo, lp->orig_upbo, sumplus, double* );
  nMALLOCCPY(newlp->orig_lowbo, lp->orig_lowbo, sumplus, double* );
  nMALLOCCPY(newlp->upbo, lp->upbo, sumplus, double* );
  nMALLOCCPY(newlp->lowbo, lp->lowbo, sumplus, double* );
  nMALLOCCPY(newlp->bas, lp->bas, rowsplus, int* );
  nMALLOCCPY(newlp->basis, lp->basis, sumplus, short int* );
  nMALLOCCPY(newlp->lower, lp->lower, sumplus, short int* );
  nMALLOCCPY(newlp->eta_value, lp->eta_value, lp->eta_alloc, double* );
  nMALLOCCPY(newlp->eta_row_nr, lp->eta_row_nr, lp->eta_alloc, int* );
  nMALLOCCPY(newlp->eta_col_end, lp->eta_col_end,
	    lp->rows_alloc + lp->max_num_inv + 1, int* );
  nMALLOCCPY(newlp->solution, lp->solution, sumplus, double* );
  nMALLOCCPY(newlp->best_solution, lp->best_solution, sumplus, double* );
  nMALLOCCPY(newlp->duals, lp->duals, rowsplus, double* );
  nMALLOCCPY(newlp->ch_sign, lp->ch_sign, rowsplus, short int* );

  if(newlp->scaling_used)
    nMALLOCCPY(newlp->scale, lp->scale, sumplus, double* );

  if(newlp->nr_lagrange > 0) {
    nMALLOCCPY(newlp->lag_rhs, lp->lag_rhs, newlp->nr_lagrange, double* );
    nMALLOCCPY(newlp->lambda, lp->lambda, newlp->nr_lagrange, double* );
    nMALLOCCPY(newlp->lag_con_type, lp->lag_con_type, newlp->nr_lagrange,
              short int* );
    nMALLOC(newlp->lag_row, newlp->nr_lagrange, double** );
    for(i = 0; i < newlp->nr_lagrange; i++)
      nMALLOCCPY(newlp->lag_row[i], lp->lag_row[i], colsplus, double* );
  }
  return(newlp);
}
...
void inc_mat_space(lprec *lp, int maxextra)
{
   if(lp->non_zeros + maxextra >= lp->mat_alloc) {
     ...
     while(lp->non_zeros + maxextra >= lp->mat_alloc)
         lp->mat_alloc += lp->mat_alloc / 2;

     nREALLOC(lp->mat, lp->mat_alloc, matrec* );
     nREALLOC(lp->col_no, lp->mat_alloc + 1, int* );
   }
}
...
void inc_row_space(lprec *lp)
{
  if(lp->rows > lp->rows_alloc) {
    ...
    nREALLOC(lp->orig_rh, lp->rows_alloc + 1, double* );
    nREALLOC(lp->rh, lp->rows_alloc + 1, double* );
    nREALLOC(lp->rhs, lp->rows_alloc + 1, double* );
    nREALLOC(lp->orig_upbo, lp->sum_alloc + 1, double* );
    nREALLOC(lp->upbo, lp->sum_alloc + 1, double* );
    nREALLOC(lp->orig_lowbo, lp->sum_alloc + 1, double* );
    nREALLOC(lp->lowbo, lp->sum_alloc + 1, double* );
    nREALLOC(lp->solution, lp->sum_alloc + 1, double* );
    nREALLOC(lp->best_solution, lp->sum_alloc + 1, double* );
    nREALLOC(lp->row_end, lp->rows_alloc + 1, int* );
    nREALLOC(lp->basis, lp->sum_alloc + 1, short int* );
    nREALLOC(lp->lower, lp->sum_alloc + 1, short int* );
    nREALLOC(lp->must_be_int, lp->sum_alloc + 1, short int* );
    nREALLOC(lp->bas, lp->rows_alloc + 1, int* );
    nREALLOC(lp->duals, lp->rows_alloc + 1, double* );
    nREALLOC(lp->ch_sign, lp->rows_alloc + 1, short int* );
    nREALLOC(lp->eta_col_end, lp->rows_alloc + lp->max_num_inv + 1, int* );
    if(lp->names_used)
      nREALLOC(lp->row_name, lp->rows_alloc + 1, char (*)[25] );
    if(lp->scaling_used)
      nREALLOC(lp->scale, lp->sum_alloc + 1, double* );
  }
}
...
void inc_col_space(lprec *lp)
{
  if(lp->columns >= lp->columns_alloc) {
    ...
    nREALLOC(lp->lower, lp->sum_alloc + 1, short int* );
    if(lp->names_used)
      nREALLOC(lp->col_name, lp->columns_alloc + 1, char (*)[25] );
    if(lp->scaling_used)
      nREALLOC(lp->scale, lp->sum_alloc + 1, double* );
    nREALLOC(lp->col_end, lp->columns_alloc + 1, int* );
  }
}
...
void str_set_obj_fn(lprec *lp, char *row)
{
  ...
  REAL *arow;
  char *p, *newp;
  nCALLOC(arow, lp->columns + 1, double* );
  p = row;
  for(i = 1; i <= lp->columns; i++) {
...
void add_constraint(lprec *lp, REAL *row, short constr_type, REAL rh)
{
  ...
  nMALLOC(addtoo, lp->columns + 1, int* );
  ...
  nMALLOC(newmat, lp->non_zeros, matrec* );
  inc_mat_space(lp, 0);
  lp->rows++;
  lp->sum++;
  inc_row_space(lp);
...
void add_lag_con(lprec *lp, REAL *row, short con_type, REAL rhs)
{
  ...
  lp->nr_lagrange++;
  if(lp->nr_lagrange == 1) {
    nCALLOC(lp->lag_row, lp->nr_lagrange, double** );
    nCALLOC(lp->lag_rhs, lp->nr_lagrange, double* );
    nCALLOC(lp->lambda, lp->nr_lagrange, double* );
    nCALLOC(lp->lag_con_type, lp->nr_lagrange, short int* );
  }
  else {
    nREALLOC(lp->lag_row, lp->nr_lagrange, double** );
    nREALLOC(lp->lag_rhs, lp->nr_lagrange, double* );
    nREALLOC(lp->lambda, lp->nr_lagrange, double* );
    nREALLOC(lp->lag_con_type, lp->nr_lagrange, short int* );
  }
  nCALLOC(lp->lag_row[lp->nr_lagrange-1], lp->columns+1, double* );
...
void str_add_lag_con(lprec *lp, char *row, short con_type, REAL rhs)
{
  ...
  REAL *a_row;
  char *p, *new_p;
  nCALLOC(a_row, lp->columns + 1, double* );
  p = row;
...
void str_set_rh_vec(lprec *lp, char *rh_string)
{
  ...
  REAL *newrh;
  char *p, *newp;
  nCALLOC(newrh, lp->rows + 1, double* );
  p = rh_string;
...
short is_feasible(lprec *lp, REAL *values)
{
  ...
  nCALLOC(this_rhs, lp->rows + 1, double* );
  if (lp->columns_scaled) {
    for(i = 1; i <= lp->columns; i++)
...
void print_lp(lprec *lp)
{
  int i, j;
  REAL *fatmat;
  nCALLOC(fatmat, (lp->rows + 1) * lp->columns, double* );
  for(i = 1; i <= lp->columns; i++)
    for(j = lp->col_end[i - 1]; j < lp->col_end[i]; j++)
...
void set_row_name(lprec *lp, int row, nstring new_name)
{
  ...
  if(!lp->names_used) {
    nCALLOC(lp->row_name, lp->rows_alloc + 1, char (*)[25] );
    nCALLOC(lp->col_name, lp->columns_alloc + 1, char (*)[25] );
    lp->names_used = TRUE;
    for(i = 0; i <= lp->rows; i++)
...
void auto_scale(lprec *lp)
{
  ...
  if(!lp->scaling_used) {
    nMALLOC(lp->scale, lp->sum_alloc + 1, double* );
    for(i = 0; i <= lp->sum; i++)
      lp->scale[i] = 1;
  }
  
  nMALLOC(row_max, lp->rows + 1, double* );
  nMALLOC(row_min, lp->rows + 1, double* );
  nMALLOC(scalechange, lp->sum + 1, double* );
...
void write_LP(lprec *lp, FILE *output)
{
  ...
  nMALLOC(row, lp->columns+1, double* );
  if(lp->maximise)
    fprintf(output, "max:");
  else
    fprintf(output, "min:");
...
void write_MPS(lprec *lp, FILE *output)
{
  ...
  nMALLOC(column, lp->rows + 1, double* );
  marker = 0;   
